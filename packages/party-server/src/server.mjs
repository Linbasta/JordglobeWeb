/**
 * JordGlobe Party Server (Production)
 *
 * WebSocket + REST API server for Cloud Run deployment.
 * Static files are served separately via Firebase Hosting.
 *
 * Run with: npm start
 */
...
import { getRandomCity, calculateDistance } from './data/cities.mjs';
import { getRandomVideo, videos } from './data/videos.mjs';
...
// CORS middleware for cross-origin requests from static client
...
app.get('/api/records', (req, res) => sendResult(res, getAllRecords()));
app.post('/api/record/reset', (req, res) => {
    const { quiz_id } = req.body;
    sendResult(res, resetRecord(quiz_id));
});

// Optional basic auth for staging (set BASIC_AUTH_USER and BASIC_AUTH_PASSWORD env vars)
if (process.env.BASIC_AUTH_USER && process.env.BASIC_AUTH_PASSWORD) {
    app.use(basicAuth({
        users: { [process.env.BASIC_AUTH_USER]: process.env.BASIC_AUTH_PASSWORD },
        challenge: true,
        realm: 'JordGlobe Party Server'
    }));
    log(`Basic auth enabled: username="${process.env.BASIC_AUTH_USER}"`);
}
...
let maxRounds = 7;          // Default number of rounds
let currentRound = 0;
        ...
        try {
            ...
            switch (message.type) {
                case 'host-connect': {
                    ...
                    // Send join URL - client provides the base URL
                    const joinUrl = message.joinUrl || null;
                    ...
                    if (joinUrl) log('Join URL:', joinUrl);
                    break;
                }
...
server.listen(PORT, '0.0.0.0', () => {
    log(`JordGlobe Party Server running on port ${PORT}`);
