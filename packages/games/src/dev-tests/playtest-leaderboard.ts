...
const DEFAULT_STORAGE_KEY = 'playtest_leaderboard_euro-music-quiz'
const ROW_COUNT = 10
...
export function showLeaderboardLocal(config: {
    ...
    const { userScore, userTotal, userElapsedMs, onClose, storageKey = DEFAULT_STORAGE_KEY } = config
    ...
    fillRows(handle, sortEntries(loadEntries(storageKey)), null)
    ...
    const submit = () => {
        ...
        const next = sortEntries([...loadEntries(storageKey), entry])
